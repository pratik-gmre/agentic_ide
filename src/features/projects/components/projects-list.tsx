...
import { AlertCircle, ArrowRight, ArrowRightIcon, GlobeIcon, Loader2 } from "lucide-react";
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <Button
        variant={"outline"}
        asChild
        className="h-auto items-start justify-start p-4 bg-background border border-border/10 rounded-none flex flex-col gap-2"
      >
...
const ProjectItems = ({ data }: { data: Doc<"projects"> }) => {
  return (
    <Link
        ...
        <div className="flex flex-col gap-2">
          <div className="flex items-center justify-between gap-2">
            ...
            <button onClick={onViewAll} className="flex items-center gap-2 text-muted-foreground text-xs hover:text-foreground transition-colors">
              ...
              <Kbd className="bg-accent border border-border/10">⌘K</Kbd>
            </button>
          </div>
          <ul>
            {rest.map((project) => (
              <ProjectItems key={project._id} data={project} />
            ))}
          </ul>
        </div>
      )}
